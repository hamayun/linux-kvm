 ...
 */
static bool kbd_in(struct ioport *ioport, struct kvm_cpu *kvm_cpu, u16 port, void *data, int size)
{
	switch (port) {
...
static bool kbd_out(struct ioport *ioport, struct kvm_cpu *kvm_cpu, u16 port, void *data, int size)
{
	switch (port) {
	case I8042_COMMAND_REG: {
		u8 value = ioport__read8(data);
		kbd_write_command(kvm_cpu->kvm, value);
		break;
	}
	case I8042_DATA_REG: {
