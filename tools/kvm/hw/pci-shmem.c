...
static bool shmem_pci__io_in(struct ioport *ioport, struct kvm_cpu *kvm_cpu, u16 port, void *data, int size)
{
...
static bool shmem_pci__io_out(struct ioport *ioport, struct kvm_cpu *kvm_cpu, u16 port, void *data, int size)
{
	u16 offset = port - ivshmem_registers;
...
static void callback_mmio_msix(struct kvm_cpu * cpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr)
{
	void *mem;
